...
var iWidth = 240;	// 日历的宽度
var iHeight= 168;	// 日历的高度 (不可变)

var _nShadowLength = 8;

var _dropDownYearLow = 25;
var _dropDownYearHeight = 10;
...
//xyq 2004.12
function ConvertName2(name)
{
	//第1个“_”替换成“:”
	//例：tbSearch_txtStart_Date 替换成 tbSearch:txtStart_Date
...
function ConvertName3(name)
{
	//例：dgList__ctl1_txtStartDate 替换成 dglist:_ctl1:txtStartDate
	l = name.length;
	for(i=0;i<l;i++)
	{
		name = ConvertName2(name);
	}
//	name = name.replace("::",":_");
// 	alert(name); 
	name = name.replace(/\:ctl/g,"_ctl");
// 	alert(name); 
...
function GetCalendar(imgClick,name, year, month, day){	
 	...
 	//alert(name); 控件中不能有下划线
 	// unm 2005.5
	name = ConvertName3(name);
	//name = "PurchaseControl21:dgDetailData:_ctl2:txtRequireDate";
...
function createFrame(){
	...
	/* table shadow   Closed by Adebris.W 2004-09-21
	if(ieVer55)
...
function InitDropYear(isFirstTime)
{
	...
	{
		for(var i = year -_dropDownYearLow;i< year +_dropDownYearHeight;i++)
		{
...
function fillCldTabFrm(year, month, day){
	...
	tableDate.rows(0).cells(0).innerHTML
	 = "<table bgcolor=#66CCFF style=\"color:white;font:bold 12px tahoma;border:0px;width:100%;height:18px;\" cellspacing=0 cellpadding=0>"
		+ "<tr><td width=10% height=22>"
		+ "&nbsp;<a style='cursor:hand' title='上' onclick=\"parent.switchLastMonth()\"><<</a>"
		+ "</td>"
