...
//			BLL.PageFacade.LoadUnitSelect(this.sltUnit,"",projectCode);
		...
		private void LoadDataGrid()
		{
			...
			try
			{
                ...
                sql = "select * from  V_ContractPayMoney ";

                QueryAgent qa = new QueryAgent();
                this.txtContractName.Value = sql;
                ...
                ud_dvContract.Sort = "ContractID Desc";
                switch (this.up_sPMNameLower)
                {
                    case "companypm":
                        this.dgList.Columns[0].Visible = true;
                        this.dgList.Columns[3].Visible = false;
                        this.dgList.Columns[4].Visible = false;
                        this.dgList.Columns[2].ItemStyle.Width = 120;
                        this.dgList.Columns[5].ItemStyle.Width = 120;
                        break;
                    default:
		...
		private void BuildSearchString()
		{
			...
			if (this.txtAdvSearch.Value != "none") 
			{
                ...
                if (this.dtAdIssueDate0.Value != "" || this.dtAdIssueDate1.Value != "")
                {
                    ArrayList ar = new ArrayList();
                    ar.Add(this.dtAdIssueDate0.Value);
                    ar.Add(this.dtAdIssueDate1.Value);
                    CSB.AddStrategy(new Strategy(ContractStrategyName.AdIssueDate, ar));
                }

            }

			//排序
			string sortsql = BLL.GridSort.GetSortSQL(ViewState, "CreateDate DESC,ContractID");

			string sql = CSB.BuildQueryViewPayMoneyString();
