...
<%@ Page Language="C#" AutoEventWireup="true" CodeFile="ContracStrikeInfo.aspx.cs" Inherits="RmsPM.Web.Contract.Contract_ContracStrikeInfo" %>

<%@ Register TagPrefix="uc1" TagName="ExchangeRate" Src="../UserControls/ExchangeRateControl.ascx" %>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.0 Transitional//EN" >
<HTML>
	<HEAD>
		<title>合同审核变更信息</title>
				...
				<tr>
					<td class="tools-area" vAlign="top"><IMG src="../images/btn_li.gif" align="absMiddle">
						<input class="button" id="btnModify" value="修 改" onclick="Modify();" type="button" name="btnModify"
							...
							type="button" name="btnCheckDelete" onserverclick="btnCheckDelete_ServerClick" runat="server" visible="false">
							 <input class="button" id="btnCheck" value="提交申请" onclick="javascript:DoCheck();" type="button"
							name="btnCheck" runat="server" onserverclick="btnCheck_ServerClick"> <input class="button" id="btnOldCheck" value="审核" onclick="javascript:if(!window.confirm('确实要批准本次结算吗？')) return false;"
							...
							runat="server" NAME="btnPrint" visible="false"> <input class="button" id="btnClose" value="关 闭" onclick="window.close();" type="button"
							name="btnClose">
					</td>
				</tr>
				<tr>
							...
							<table class="form" cellSpacing="0" cellPadding="0" width="100%" border="0">
								...
								<tr>
									...
									<td><igtxt:webnumericedit id="txtSupplierChangeMoney" runat="server" JavaScriptFileNameCommon="../images/infragistics/20051/scripts/ig_shared.js"
											...
											CssClass="infra-input-nember" MinDecimalPlaces="Two" Width="100" readonly></igtxt:webnumericedit></td>
								</tr>
								<tr>
...
function Modify()
{
	window.location.href = "../Contract/ContracStrikeModify.aspx?ProjectCode=<%=Request["ProjectCode"]%>&Act=Edit&ContractCode=<%=Request["ContractCode"]%>&ContractChangeCode=<%=Request["ContractChangeCode"]%>";
}
...
function DoCheck()
{
	...
	OpenFullWindow('../WorkFlowPage/sm_contractaccountauditing.aspx?ProjectCode=<%=Request["ProjectCode"]%>&ContractCode=<%=Request["ContractCode"]%>&ApplicationCode1=<%=Request["ContractCode"]%>&ContractChangeCode=<%=Request["ContractChangeCode"]%>','合同结算_<%=Request["ContractCode"]%>');
}
