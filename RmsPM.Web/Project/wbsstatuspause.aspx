...
<%@ Page language="c#" Inherits="RmsPM.Web.Project.WBSStatusPause" CodeFile="WBSStatusPause.aspx.cs" %>
	...
	<HEAD>
		...
		<script>
				<!--
					...
					{
						window.location.href = "WBSStatusPause.aspx?" + Condition +"&TaskStatus=<%=Request["TaskStatus"]%>&ProjectCode=<%=Request["ProjectCode"]%>&TaskExceed=" + document.all.TaskExceed.value;
					}
						...
						<asp:datagrid id="dgTaskList" runat="server" DataKeyField="WBSCode" AutoGenerateColumns="False"
							...
							<Columns>
								...
								<asp:BoundColumn DataField="PlannedFinishDate" SortExpression="PlannedFinishDate" HeaderText="计划结束时间"
									DataFormatString="{0:yyyy-MM-dd}"></asp:BoundColumn>
								<asp:BoundColumn DataField="PauseReason" HeaderText="暂停原因" DataFormatString="{0:10}"></asp:BoundColumn>
								<asp:BoundColumn DataField="PauseDate" HeaderText="暂停时间" DataFormatString="{0:yyyy-MM-dd}"></asp:BoundColumn>
							</Columns>
