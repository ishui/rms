...
namespace RmsPM.Web.Project
{
	...
	/// 工作分解树导入
	/// </summary>
	public partial class TaskTempletIn : PageBase
	{
		...
		private void InData(string templetCode)
		{
			...
			//项目结点
			string ProjectWBSCode = BLL.TaskRule.GetProjectTaskCodeByProject(projectCode);
			...
			entity = WBSDAO.GetStandard_WBSTempletByCode(templetCode);
			if (entity.HasRecord())
			{
				...
				*/
				...
				for ( int i=0;i<iCount;i++)
				{
					...
					{
						newParentCode = RootWBSCode;
						newParentFullCode = RootWBSCode;
				...
				string strUser = "";
				string strStation = "";
				EntityData entityUser = WBSDAO.GetTaskPersonByWBSCode(RootWBSCode);
//				EntityData entityUser = WBSDAO.GetTaskPersonByWBSCode(ProjectWBSCode);
				...
				//权限加入当前用户
				if (strUser.IndexOf(base.user.UserCode) < 0) 
				{
					strUser +=(strUser == "")?"":",";
				...
				for(int i=0;i< iCount ;i++)
				{
					...
					newTaskRow["FullCode"] = RootParentFullCode + "-" + BLL.ConvertRule.ToString(drs[i]["NewFullCode"]);
					...
					newTaskRow["Deep"] = RootDeep + BLL.ConvertRule.ToInt(drs[i]["Deep"]);
