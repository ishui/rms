<%@ Page language="c#" Inherits="RmsPM.Web.Project.WBSStatusOverTime" CodeFile="WBSStatusOverTime.aspx.cs" %>
	...
	<HEAD>
		...
		<script>
				<!--
					...
					{
						window.location.href = "WBSStatusOverTime.aspx?" + Condition +"&TaskStatus=<%=Request["TaskStatus"]%>&ProjectCode=<%=Request["ProjectCode"]%>&TaskExceed=" + document.all.TaskExceed.value;
					}
							...
							<asp:datagrid id="dgTaskList" runat="server" DataKeyField="WBSCode" AutoGenerateColumns="False"
								...
								<Columns>
									...
									<asp:BoundColumn DataField="ActualStartDate" SortExpression="ActualStartDate" HeaderText="实际开始时间"
										DataFormatString="{0:yyyy-MM-dd}"></asp:BoundColumn>
									<asp:BoundColumn DataField="ActualFinishDate" SortExpression="ActualFinishDate" HeaderText="实际结束时间"
										DataFormatString="{0:yyyy-MM-dd}"></asp:BoundColumn>
								</Columns>
