namespace RmsPM.Web.PBS
{
	...
	/// SearchRoom 查询
	/// </summary>
	public partial class SearchRoom : System.Web.UI.UserControl
	{
		...
		public void SetProject(string ProjectCode)
		{
			try 
			{
				...
				PageFacade.LoadPBSTypeSelectAll(sltSearchPBSTypeCode,"",this.txtProjectCode.Value);
		...
		public void SetDefaultInvState(string InvState) 
		{
			try 
			{
				this.sltSearchInvState.Value = InvState;
			}
			catch ( Exception ex )
			{
				ApplicationLog.WriteLog(this.ToString(),ex,"设置缺省的查询条件：库存状态失败");
				Response.Write(Rms.Web.JavaScript.Alert(true, "设置缺省的查询条件：库存状态失败：" + ex.Message));
			}
		}
		...
		/// </summary>
		/// <param name="InvState"></param>
		public void SetDefaultPBSTypeCode(string PBSTypeCode) 
		{
			try 
			{
				this.sltSearchPBSTypeCode.Value = PBSTypeCode;
			}
			catch ( Exception ex )
			{
				ApplicationLog.WriteLog(this.ToString(),ex,"设置缺省的查询条件：产品类型失败");
				Response.Write(Rms.Web.JavaScript.Alert(true, "设置缺省的查询条件：产品类型失败：" + ex.Message));
			}
		}
