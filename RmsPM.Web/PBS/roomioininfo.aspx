<%@ Page language="c#" Inherits="RmsPM.Web.PBS.RoomIOInInfo" CodeFile="RoomIOInInfo.aspx.cs" %>
			...
			<table height="100%" cellSpacing="0" cellPadding="0" width="100%" border="0">
				...
				<TR>
					<TD class="tools-area" valign="top">
						...
						<input style="display:none" class="button" id="btnPrint" onclick="Print();" type="button" value="打 印" name="btnPrint"
							runat="server">
						<input class="button" id="btnExcel" type="button" value="Excel" name="btnExcel" runat="server"
							onclick="if (!Excel()) return false;" onserverclick="btnExcel_ServerClick">
						<IMG src="../images/btn_li.gif" align="absMiddle">
						<input class="button" id="btnModify" onclick="Modify();" type="button" value="修 改" name="btnModify"
							runat="server">
						<input class="button" id="btnDelete" onclick="javascript:if(!window.confirm('确实要删除这条记录吗？')) return false;Delete();"
							type="button" value="删 除" name="btnDelete" runat="server">
						<input class="button" id="btnCheck" onclick="javascript:if(!window.confirm('审核后将不能再修改，确实要审核吗？')) return false;DoCheck();"
							type="button" value="审 核" name="btnCheck" runat="server">
						<input class="button" id="btnCancelCheck" onclick="javascript:if(!window.confirm('确实要取消审核吗？')) return false;DoCancelCheck();"
					...
					<td class="table" valign="top">
						<TABLE class="form" id="Table4" cellSpacing="0" cellPadding="0" width="100%">
							...
							<TR>
								...
								<TD><asp:label id="lblConferMark" Runat="server"></asp:label></TD>
							</TR>
							<TR>
...
function Modify()
{
	...
	window.location.href = "RoomIOInModify.aspx?FromUrl=" + escape(window.location.href) + "&OutListCode=" + Form1.txtOutListCode.value + "&act=Modify";
}
...
{
	document.all.divHintSave.style.display = "";
	window.location.href = "RoomIOInModify.aspx?FromUrl=" + escape(Form1.txtFromUrl.value) + "&OutListCode=" + Form1.txtOutListCode.value + "&act=delete";
}

//打印
function Print()
{
	OpenPrintWindow("../Product/ProductBillrealList.aspx?Code="+Form1.txtOutListCode.value, "打印");
}
