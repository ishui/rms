	...
	public partial class RoomIOInfo : PageBase
	{
		...
		private void IniPage() 
		{
			try 
			{
				...
				//权限
				if (this.txtOutState.Value == "退库") 
				{
					this.btnModify.Visible = base.user.HasRight("011403");
					this.btnDelete.Visible = base.user.HasRight("011404");
					this.btnCheck.Visible = base.user.HasRight("011405");
					this.btnCancelCheck.Visible = base.user.HasRight("011405");
				}
				else 
				{
					this.btnModify.Visible = base.user.HasRight("011303");
					this.btnDelete.Visible = base.user.HasRight("011304");
					this.btnCheck.Visible = base.user.HasRight("011305");
					this.btnCancelCheck.Visible = base.user.HasRight("011305");
				}
			...
			try 
			{
				...
				if (OutListCode != "") 
				{
					...
					if (entity.HasRecord()) 
					{
						...
						//隐藏去向
						if ((this.txtOutState.Value != "出库") && (this.txtOutState.Value != "调拨") )
						{
//							this.tdToolSep1.Visible = false;
							this.btnPrint.Visible = false;
//							this.btnExcel.Visible = false;

							this.spanOutAspect.Visible = false;
							this.lblOutAspect.Visible = false;
						}
					}
					else 
					{
...
//				}
				...
				try 
				{
					...
					//新建工作簿
					if (this.txtOutState.Value == "退库")
						excel.TemplateFileName = "房屋退库单.xls";
					else
						excel.TemplateFileName = "房屋调拨通知单.xls";
