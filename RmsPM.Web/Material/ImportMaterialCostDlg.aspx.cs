...
{
	/// <summary>
	/// ImportMaterialCostDlg 的摘要说明。
	/// </summary>
	public partial class ImportMaterialCostDlg : PageBase
	{
		...
		private void IniPage() 
		{
			try 
			{
				BLL.MaterialCostImport imp = new MaterialCostImport();
				...
				try 
				{
					DataTable tbAllSystemGroup = qa.ExecSqlForDataSet("select dbo.GetSystemGroupFullName(GroupCode) as FullName, * from SystemGroup where ClassCode = '1411'").Tables[0];
		...
		protected void btnOK_ServerClick(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				BLL.MaterialCostImport imp = new MaterialCostImport();
				EntityData entity = DAL.EntityDAO.MaterialDAO.GetAllMaterialCost();
				...
				while (m_sr.Peek() >= 0) 
				{
					...
					string hint = "";
					string Description = "";
					string MaterialCostGroupFullName = "";
					DataRow dr = imp.ImportMaterialCostSingle(s, entity, ref hint, ref Description, ref MaterialCostGroupFullName, false, (DataTable)Session["tbAllSystemGroup"]);
			...
			//导入有错时记日志
			ApplicationLog.WriteFile("材料价格导入.log", this.ToString(), this.txtResult.Value);
		...
		protected void btnClear_ServerClick(object sender, System.EventArgs e)
		{
			...
			{
				//清空材料价格
				BLL.MaterialRule.DeleteAllMaterialCost();
			}
			catch(Exception ex)
			{
				ApplicationLog.WriteLog(this.ToString(),ex,"");
			...
			Response.Write(JavaScript.Alert(true,"材料价格已清空"));
			RefreshParent();
		}
		...
		private void ReadStream()
		{
			...
			Session["tbImportMaterialCostStream"] = tbStream;
		}
		...
		private void ShowImportList()
		{
			try 
			{
				if (Session["tbImportMaterialCostStream"] == null)
					throw new Exception("超时，请重新导入");
					
				DataTable tbStream = (DataTable)Session["tbImportMaterialCostStream"];

				BLL.MaterialCostImport imp = new MaterialCostImport();
//				EntityData entity = new EntityData("MaterialCost");
				EntityData entity = DAL.EntityDAO.MaterialDAO.GetAllMaterialCost();
				...
				foreach(DataRow drStream in tbStream.Rows)
				{
                    ...
                    DataRow dr = imp.ImportMaterialCostSingle(s, entity, ref hint, ref Description, ref GroupFullName, true, (DataTable)Session["tbAllSystemGroup"]);
					...
					if (dr == null) 
					{
						...
						dr["MaterialCostCode"] = "ERR_" + r.ToString();
						dr["ImportHint"] = hint;
						dr["ImportResult"] = -1;
				...
				Session["entityImportMaterialCost"] = entity;
				BindImportList();
//				entity.Dispose();
			}
			catch(Exception ex)
			{
				ApplicationLog.WriteLog(this.ToString(),ex,"");
				Response.Write(JavaScript.Alert(true, "显示要导入的材料价格列表出错：" + ex.Message));
			}
		}

		private void BindImportList()
		{
			EntityData entity = (EntityData)Session["entityImportMaterialCost"];
			...
			try 
			{
				...
				try
				{
					...
					foreach(DataRow drStream in tbStream.Rows)
					{
                        ...
                        string Description = "";
						string GroupFullName = "";
                        DataRow dr = imp.ImportMaterialCostSingle(s, entity, ref hint, ref Description, ref GroupFullName, false, (DataTable)Session["tbAllSystemGroup"]);
		...
		protected void btnDownloadCsv_ServerClick(object sender, System.EventArgs e)
		{
			try 
			{
				if (Session["entityImportMaterialCost"] == null)
					throw new Exception("超时，请重新导入");
				...
				io.SaveFileName = "MaterialCostImportPrepare.csv";
