...
using RmsPM.Web;
	...
	/// </summary>
public partial class WorkFlowPage_SM_ContractAccountAuditing : WorkFlowPageBase
{
    ...
    protected void Page_Load(object sender, System.EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            this.WorkFlowName = "合同结算审核";// System.Configuration.ConfigurationSettings.AppSettings["ContractAccountAuditingName"].ToString();
    ...
    override protected void PageControlInit()
    {
        ...
        string ud_sContractAccountCode = "";

        if (this.wftToolbar.ApplicationCode != "")
        {
            ud_sContractAccountCode = this.wftToolbar.ApplicationCode;
            ud_sContractCode = RmsPM.BLL.ContractRule.GetContractCodeByAccountCode(ud_sContractAccountCode);
...
//        this.up_ucOperationControl.MoneyState = this.wftToolbar.GetModuleState("金额");
        ...
        OpinionControlInit("意见", "SM_CAA_工程部", "工程部", this.wfoOpinion1);
  
        OpinionControlInit("意见", "SM_CAA_合约部", "合约部", this.wfoOpinion2);

        //总部总监
      
        OpinionControlInit("工程总监", "SM_CAA_工程总监", "工程总监", this.wfoOpinion3);
       
        OpinionControlInit("合约总监", "SM_CAA_合约总监", "合约总监", this.wfoOpinion4);
       
        OpinionControlInit("财务总监", "SM_CAA_财务总监", "财务总监", this.wfoOpinion5);

        //董事会
    
        OpinionControlInit("工程执董", "SM_CAA_工程执董", "工程执董", this.wfoOpinion6);
      
        OpinionControlInit("合约执董", "SM_CAA_合约执董", "合约执董", this.wfoOpinion7);
    
        OpinionControlInit("财务执董", "SM_CAA_财务执董", "财务执董", this.wfoOpinion8);
       
        OpinionControlInit("销售执董", "SM_CAA_销售执董", "销售执董", this.wfoOpinion11);
        ...
        else
        {
            ...
            OpinionControlInit("项目副总监", "SM_CAA_项目副总监", "项目副总监", this.wfoOpinion9);
       
            OpinionControlInit("项目总监", "SM_CAA_项目总监", "项目总监", this.wfoOpinion10);
