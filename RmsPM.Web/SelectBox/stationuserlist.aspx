<%@ Page language="c#" Inherits="RmsPM.Web.SelectBox.StationUserList" CodeFile="StationUserList.aspx.cs" %>
								...
								<asp:Repeater ID="repeaterSU" Runat="server">
									<ItemTemplate>
										<tr>
											...
											<td><input type=checkbox id=chk  NameTemp='<%# DataBinder.Eval( Container,"DataItem.Name" )%>' RelationCode='<%# DataBinder.Eval( Container,"DataItem.RelationCode" )%>' AccessRangeType='<%# DataBinder.Eval( Container,"DataItem.AccessRangeType" )%>' ></td>
			...
			function getSelectSU()
			{
				...
				if ( obj[0])
				{
					for ( var i=0;i<obj.length;i++)
					{
						if ( obj[i].checked )
						{
							var aaa = new AccessRange( obj[i].RelationCode , obj[i].AccessRangeType ,obj[i].NameTemp ) ;
							ar.push(aaa);
						}
					}
				}
				else
				{
					if ( obj.checked)
					{
						var aaa = new AccessRange( obj.RelationCode , obj.AccessRangeType ,obj.NameTemp ) ;
						ar.push(aaa);
					}
				}
