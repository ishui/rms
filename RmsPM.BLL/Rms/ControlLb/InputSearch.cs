namespace Rms.ControlLb
{
    ...
    [DefaultProperty("Text"), ToolboxData("<{0}:InputSearch runat=server></{0}:InputSearch>")]
    public class InputSearch : WebControl
    {
        ...
        protected string OpenMiddleWindows()
        {
            ...
            builder.Append("function OpenNewSelectBox()\n{\n{" + this.ReturnMiddleWindowsScript(this.Url + "?ReturnValue=GetReturnValue&xmlUrl=" + this.XmlUrl + "&ProjectCode=" + this.ProjectCode) + "\n}return false;}\n");
        ...
        protected override void Render(HtmlTextWriter output)
        {
            ...
            output.Write("<input type=\"hidden\" value=" + this.ValueID + "  id=\"" + this.UniqueID + "ID\" name=\"" + this.UniqueID + "ID\" >");
        ...
        public string ValueID
        {
            get
            {
                string text = (string) this.ViewState["ID"];
                return ((text == null) ? string.Empty : text);
            }
            set
            {
