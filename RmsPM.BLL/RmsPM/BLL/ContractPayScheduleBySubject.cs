namespace RmsPM.BLL
{
    ...
    public class ContractPayScheduleBySubject
    {
        ...
        public ContractPayScheduleBySubject(string a_ProjectCode)
        {
        ...
        private void AddRelationContractRow(DataTable tbContract, DataTable tbSrc, string RecordType, string MoneyField)
        {
            try
            {
                foreach (DataRow row in tbSrc.Rows)
                {
                    ...
                    string text = ConvertRule.ToString(row["ContractCode"]);
                    string text2 = ConvertRule.ToString(row["CostCode"]);
                    DataRow[] rowArray = tbContract.Select("ContractCode = '" + text + "' and CostCode='" + text2 + "'");
                    if (rowArray.Length == 0)
                    {
                    ...
                    row2[MoneyField] = ConvertRule.ToDecimal(row["Money"]);
                    if (tbSrc.Columns.Contains("ContractPayReal"))
                    {
                        row2["ContractPayReal"] = ConvertRule.ToDecimal(row2["ContractPayReal"]) + ConvertRule.ToDecimal(row["ContractPayReal"]);
                    }
        ...
        private void CreateTable()
        {
            try
            {
                ...
                table.Columns.Add("IsExistsContract", typeof(int));
        ...
        public static string GetContractPayHref(object Money, object SubjectCode, object ContractCode, object IsContract, object PBSType, object PBSCode)
        {
            string text2;
            try
            {
                ...
                {
                    text = string.Format("<a href=\"#\" onclick=\"ViewContractPBSPayBySubject('{1}', '{2}', '{3}', '{4}', '{5}');return false;\">{0}</a>", new object[] { ConvertRule.ToString(Money), ConvertRule.ToString(SubjectCode), ConvertRule.ToString(ContractCode), ConvertRule.ToString(IsContract), ConvertRule.ToString(PBSType), ConvertRule.ToString(PBSCode) });
                }
                text2 = text;
            }
            catch (Exception exception)
        ...
        public static string GetContractPayHref(object objRecordType, object Money, object CostCode, object ContractCode, object IsContract, object PBSType, object PBSCode)
        {
            ...
            try
            {
                ...
                if (ConvertRule.ToString(objRecordType) != "Apportion")
                {
                    text = GetContractPayHref(Money, CostCode, ContractCode, IsContract, PBSType, PBSCode);
                }
                else
                {
        ...
        public static string GetContractPayRealBalanceHref(object Money, object SubjectCode, object ContractCode, object IsContract, object PBSType, object PBSCode)
        {
            string text2;
            try
            {
                ...
                if (ConvertRule.ToString(Money) != "")
                {
                    text = string.Format("<a href=\"#\" onclick=\"ViewContractPBSPayRealBalanceBySubject('{1}', '{2}', '{3}', '{4}', '{5}');return false;\">{0}</a>", new object[] { ConvertRule.ToString(Money), ConvertRule.ToString(SubjectCode), ConvertRule.ToString(ContractCode), ConvertRule.ToString(IsContract), ConvertRule.ToString(PBSType), ConvertRule.ToString(PBSCode) });
                }
                text2 = text;
            }
            catch (Exception exception)
        ...
        public static string GetContractPayRealBalanceHref(object objRecordType, object Money, object CostCode, object ContractCode, object IsContract, object PBSType, object PBSCode)
        {
            ...
            try
            {
                ...
                if (ConvertRule.ToString(objRecordType) != "Apportion")
                {
                    text = GetContractPayRealBalanceHref(Money, CostCode, ContractCode, IsContract, PBSType, PBSCode);
                }
                else
                {
                    text = ConvertRule.ToString(Money);
        ...
        public static string GetContractPayRealHref(object Money, object SubjectCode, object ContractCode, object IsContract, object PayoutDateBegin, object PayoutDateEnd, object PBSType, object PBSCode)
        {
            string text2;
            try
            {
        ...
        public static string GetContractPayRealHref(object objRecordType, object Money, object CostCode, object ContractCode, object IsContract, object PayoutDateBegin, object PayoutDateEnd, object PBSType, object PBSCode)
        {
            ...
            try
            {
                ...
                if (ConvertRule.ToString(objRecordType) != "Apportion")
                {
                    text = GetContractPayRealHref(Money, CostCode, ContractCode, IsContract, PayoutDateBegin, PayoutDateEnd, PBSType, PBSCode);
                }
                else
                {
                    text = ConvertRule.ToString(Money);
            ...
            try
            {
                ...
                foreach (DataRowView view2 in view)
                {
                    ...
                    if (ConvertRule.ToString(dr["FullCode"]) != "")
                    {
                        ...
                        dr["IsExistsContract"] = (drs.Length > 0) ? 1 : 0;
                        string[] textArray = new string[] { "ContractPayReal" };
            ...
            try
            {
                ...
                if ((this.StartYm != "") && (this.EndYm != ""))
                {
                    foreach (DataRow row in this.tb.Rows)
                    {
                        ...
                        string subjectCode = ConvertRule.ToString(row["CostCode"]);
                        ...
                        for (int i = 0; i < monthsBetween; i++)
                        {
                            ...
                            if (pBSCode == "")
                            {
                                text12 = GetContractPayRealHref(money, subjectCode, contractCode, "", s, payoutDateEnd, this.PBSType, pBSCode);
                            }
                            else
                            {
                                text12 = money;
